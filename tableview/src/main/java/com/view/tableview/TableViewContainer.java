...
public class TableViewContainer extends LinearLayout {
    /**
     * 上下文
     */
     ...
     * @param mTableDatas
     */
    public TableViewContainer(Context mContext, ArrayList<ParentEntity> mTableDatas) {
        super(mContext);
    ...
    public void setContentView() {
        removeAllViews();
        View itemView = LayoutInflater.from(mContext).inflate(R.layout.table_content_view, null);
        initView(itemView);
        initData();
        addView(itemView);
    }
    ...
    private void initView(View itemView) {
        ...
        if (mOnTableViewCreatedListener != null) {
            mOnTableViewCreatedListener.onTableViewCreatedCompleted(mScrollView);
        }
    }

    private void initData() {
        //构造锁定视图
        mLockRecyclerView.setVisibility(View.VISIBLE);
        if (mLockAdapter == null) {
            ...
            mLockRecyclerView.addItemDecoration(decoration);
            mLockRecyclerView.setAdapter(mLockAdapter);
        } else {
            mLockAdapter.notifyDataSetChanged();
        }
        ...
        if (mUnLockAdapter == null) {
            ...
            mMainRecyclerView.setAdapter(mUnLockAdapter);
        } else {
            mUnLockAdapter.notifyDataSetChanged();
        }
    }
